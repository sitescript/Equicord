...
async function generateDebugInfoMessage() {
    ...
    const commonIssues = {
        ":trollmark: NoRPC enabled": Vencord.Plugins.isPluginEnabled("NoRPC"),
        ":trollmark: Activity Sharing disabled": tryOrElse(() => !ShowCurrentGame.getSetting(), false),
        ":trollmark: :Dev: Equicord DB": !IS_STANDALONE,
        ":trollmark: :User: Has UP'S": Object.values(PluginMeta).some(m => m.userPlugin),
        ":lastfm404:  More than two weeks out of date": BUILD_TIMESTAMP < Date.now() - 12096e5,
    };
